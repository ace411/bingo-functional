...
function either(callable $left, callable $right, $either)
{
...
function _extract(array $eithers, string $class)
{
    $extract = compose(
        partialRight(
            'array_filter', 
            function ($either) use ($class) { 
                return $either instanceof $class; 
            }
        ),
        partialLeft(
            'array_map', 
...
function fromRight($default, $either)
{
